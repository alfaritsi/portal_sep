$(document).ready(function () {
    ...
    $(document).on("click", "button[name='action_btn']", function (e) {
        ...
        if (valid) {
            ...
            if (isproses == 0) {
                ...
                $.ajax({
                    url: baseURL + 'spk/master/save/namaspk',
                    type: 'POST',
                    dataType: 'JSON',
                    data: formData,
                    contentType: false,
    ...
    $(document).on('click', '.edit', function (e) {
        ...
        $.ajax({
            url: baseURL + 'spk/master/get/namaspk',
            type: 'POST',
            dataType: 'JSON',
            data: {
                id: id
            ...
            success: function (data) {
                ...
                if (data.data) {
                    ...
                    $('input[name="id_nama_spk"]').val(id);
                    $('#id_jenis_spk').val(dataEdit.id_jenis_spk);
                    $('#nama_spk').val(dataEdit.nama_spk);

                    var selected = [];
                    for (var divisi of dataEdit.divisis) {
                        selected.push(divisi.id_divisi)
                    }

                    $('#divisis').val(selected);
                    $('#divisis').trigger('change');
        ...
        kiranaConfirm(
            {
                ...
                successCallback: function () {
                    $.ajax({
                        url: baseURL + 'spk/master/delete/namaspk',
                        type: 'POST',
                        dataType: 'JSON',
                        data: {
                            id: id
        ...
        kiranaConfirm(
            {
                ...
                successCallback: function () {
                    $.ajax({
                        url: baseURL + 'spk/master/set/namaspk',
                        type: 'POST',
                        dataType: 'JSON',
                        data: {
                            id: id,
